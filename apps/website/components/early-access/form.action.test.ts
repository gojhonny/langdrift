...
vi.mock('./turnstile.validation', () => ({
  verifyEarlyAccessChallenge: vi.fn()
}))
...
describe('server submission', () => {
  it('blocks invalid challenges before Go and strips the token from the payload', async () => {
    vi.mocked(verifyEarlyAccessChallenge)
      .mockResolvedValueOnce(false)
      .mockResolvedValueOnce(true)
    expect(await submitEarlyAccess(input)).toEqual({
      ok: false,
    ...
    expect(axios.post).toHaveBeenCalledWith(
      'https://example.invalid/v1/emails',
      { email: input.email, locale: input.locale, source: input.source },
      expect.objectContaining({ timeout: 4000, maxRedirects: 0 })
    )
  })

  it('handles validation and broker failures without exposing provider detail', async () => {
    expect(await submitEarlyAccess({ ...input, email: 'bad' })).toMatchObject({
      ok: false,
  ...
  it('rejects malformed token before verification', async () => {
    expect(
      await submitEarlyAccess({ ...input, turnstileToken: 'x'.repeat(2049) })
    ).toEqual({ ok: false, code: 'CHALLENGE_FAILED' })
    expect(
    ...
    expect(await submitEarlyAccess({ ...input, turnstileToken: null })).toEqual(
