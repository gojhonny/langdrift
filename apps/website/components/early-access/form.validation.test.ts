...
import fixtures from '@event-testdata/registration.json'
...
describe('early access submission', () => {
  it.each(
    fixtures
  )('shares Go validation and identity for $email', (fixture) => {
    const result = earlyAccessSubmissionSchema.safeParse({
      email: fixture.email,
      locale: 'en',
      source: 'landing',
  ...
  it('rejects unknown fields, invalid email and oversized tokens', () => {
    ...
    expect(
      earlyAccessSubmissionSchema.safeParse({ ...valid, extra: true }).success
    ).toBe(false)
    expect(
      earlyAccessSubmissionSchema.safeParse({ ...valid, email: 'invalid' })
        .success
    ).toBe(false)
    expect(
      earlyAccessSubmissionSchema.safeParse({
        ...valid,
