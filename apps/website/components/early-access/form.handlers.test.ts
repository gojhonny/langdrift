...
import type {
  EarlyAccessActionResult,
  EarlyAccessFormState,
  StateUpdater
} from './form.types'
  ...
  it.each([
    ...
    if (!response.ok && response.code === 'VALIDATION_ERROR') {
      expect(state.fieldErrors.email).toBe(
        response.fieldErrors.email === 'emailTooLong'
          ? 'emailTooLong'
          : 'emailInvalid'
      )
    }
