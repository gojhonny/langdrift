...
vi.mock('next/script', () => ({ default: ({ onReady }: { onReady: () => void }) => {
...
beforeEach(() => {
  vi.clearAllMocks()
  window.turnstile = {
    render: vi.fn((_element, options) => { options.callback('challenge-token'); return 'widget-id' }),
    reset: vi.fn()
  }
})
afterEach(cleanup)
...
describe('early access form', () => {
  it('submits a local email with its challenge token and clears on acceptance', async () => {
    ...
    render(<EarlyAccessForm copy={earlyAccessMessages.en} helperId="helper" locale="en" source="landing" />)
    ...
    await waitFor(() => expect(submitEarlyAccess).toHaveBeenCalledWith({ email: 'a@example.com', locale: 'en', source: 'landing', turnstileToken: 'challenge-token' }))
    await waitFor(() => expect(screen.getByText(earlyAccessMessages.en.success)).toBeTruthy())
  ...
  it('does not submit without a challenge', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Email address'), { target: { value: 'a@example.com' } })
    fireEvent.click(screen.getByRole('button', { name: /join early access/i }))
    expect(screen.getByText(earlyAccessMessages.en.challengeFailed)).toBeTruthy()
  ...
  it('validates on blur and keeps invalid email out of the Server Function', async () => {
    ...
    expect(screen.getByText(earlyAccessMessages.en.errors.emailInvalid)).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: /join early access/i }))
    expect(submitEarlyAccess).not.toHaveBeenCalled()
  })

  it('shows challenge failure and resets the spent widget', async () => {
    vi.mocked(submitEarlyAccess).mockResolvedValue({ ok: false, code: 'CHALLENGE_FAILED' })
    render(<EarlyAccessForm copy={earlyAccessMessages.en} helperId="helper" locale="en" source="landing" />)
    await waitFor(() => expect(window.turnstile?.render).toBeDefined())
    fireEvent.change(screen.getByLabelText('Email address'), { target: { value: 'a@example.com' } })
    fireEvent.click(screen.getByRole('button', { name: /join early access/i }))
    await waitFor(() => expect(screen.getByText(earlyAccessMessages.en.challengeFailed)).toBeTruthy())
  ...
  it('renders the pricing form with its own source', () => {
    ...
    expect(screen.getByText(earlyAccessMessages['pt-BR'].pricingHeading)).toBeTruthy()
  })

  it('shows an accessible retry message after a broker failure', async () => {
    vi.mocked(submitEarlyAccess).mockResolvedValue({ ok: false, code: 'UNAVAILABLE' })
    render(<EarlyAccessSection locale="en" source="landing" />)
    await waitFor(() => expect(window.turnstile?.render).toHaveBeenCalled())
    fireEvent.change(screen.getByLabelText('Email address'), { target: { value: 'a@example.com' } })
    fireEvent.click(screen.getByRole('button', { name: /join early access/i }))
    await waitFor(() => expect(screen.getByText(earlyAccessMessages.en.unavailable)).toBeTruthy())
