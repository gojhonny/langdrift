...
[ "$#" -eq 0 ] || drift_die "Usage: drift verify packages" 2
...
drift_need awk
drift_need node
drift_loader_start "Verifying packages"
drift_queue_scene "$DRIFT_ICON_VERIFY" "$DRIFT_VERIFY_PHRASE"

package_fixture=$(mktemp -d "${TMPDIR:-/tmp}/langdrift-packages.XXXXXX")
trap 'rm -rf "$package_fixture"; drift_flush_results' EXIT
...
TS
"$DRIFT_PROJECT_ROOT/node_modules/.bin/tsc" --noEmit --strict --skipLibCheck --module NodeNext --moduleResolution NodeNext --target ES2023 consumer.ts
