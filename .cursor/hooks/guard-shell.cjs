...
process.stdin.on('end', () => {
  ...
  try {
    ...
    respond(
      'ask',
      'LangDrift guard could not parse the shell-hook payload. Review the command manually.'
    )
    return
  }
  ...
  if (!command) {
    respond(
      'ask',
      'LangDrift guard received no shell command. Review the operation manually.'
    )
    return
  }
  ...
  const denyRules = [
    ...
    [
      ...
      'Force-pushing is blocked by the LangDrift agent guard.'
    ],
    [
